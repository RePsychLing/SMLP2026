...
- fit a linear mixed model with a correlated random intercept and slope using MixedModels.jl;
- interpret the fixed effects, variance components, and correlation parameter;
- compare models with and without a random slope.
:::

::: {.callout-note title="Before you start"}
**Prerequisites:** [Setup: installing the tools](../getting-started/setup.qmd).
...
::: {.callout-note collapse="true" title="Small infelicities"}
This standard description is slightly inaccurate.
...
See [lme4 GitHub issue #615](https://github.com/lme4/lme4/issues/615) for more information.
:::

# Loading the data

First load the MixedModels package and other packages for plotting.
...
The `sleepstudy` data are one of the datasets available with the `MixedModels` package.  It is re-exported by the `SMLP2026` package's `dataset` function.

```{julia}
sleepstudy = DataFrame(dataset("sleepstudy"))
```

@fig-reaction_vs_days_by_subject displays the data in a multi-panel plot.
...
#| echo: false
m1 = let f = @formula(reaction ~ 1 + days + (1 + days | subj))
  fit_or_restore("sleepstudy_m1.json", MixedModel, f, sleepstudy)
end
```
...
Similarly within-subject slopes would be expected to have a range of about 0 ms per day up to 20 ms per day.
...
caterpillar!(Figure(; size=(600, 400)), m1; vline_at_zero=true)
```

::: {.callout-note}
This plot could be drawn as `caterpillar(m1)`.
The reason for explicitly creating a `Figure` to be modified by `caterpillar!` is to control the resolution.
:::
...
#| echo: false
m2 = let f = @formula reaction ~ 1 + days + zerocorr(1 + days | subj)
  fit_or_restore("sleepstudy_m2.json", MixedModel, f, sleepstudy)
end
```
...
This section is a first taste of the parametric bootstrap; the full treatment is in the [Inference & Uncertainty section](../inference/bootstrap.qmd).
...
::: {.callout-note}
MixedModels.jl also defines appropriate methods for `StatsAPI.confint` so that the above example could be written as

```{julia}
confint(m1bstp; method=:shortest)
```

The parametric bootstrap is treated in full in [Parametric bootstrap for mixed-effects models](../inference/bootstrap.qmd); [confidence intervals from the profiled objective](../inference/profiling.qmd) cover an alternative based on the likelihood surface.
:::
...
let
  ...
  contour!(kde((tbl.β1, tbl.β2)); color=:black)
  current_figure()
end
```
...
- [The Belenky Sleep Study: Speed Metrics and Between-Group Comparisons](../contrasts/sleepstudy_speed.qmd) — re-analyzes a related sleep-deprivation dataset using a speed (1/RT) transformation of the response.
- [Convergence, Singularity, and All That](singularity.qmd) — uses the sleepstudy data to illustrate what happens when a model's random-effects covariance structure is degenerate.
- [Parametric bootstrap for mixed-effects models](../inference/bootstrap.qmd) — the full treatment of the bootstrap previewed above.
- The [introductory chapter](https://www.embraceuncertaintybook.com/intro.html) of *Embrace Uncertainty* — the conceptual foundations of mixed models (fixed vs. random effects, maximum likelihood, conditional modes) on the simplest possible example.
...
Work through these by running the code yourself; expand the solution only after you have an answer.

1. **Centering.** Refit the model with `days` centered at the baseline (day 2 in the corrected description) instead of day 0. What changes — the slope, the intercept, or both — and how does the intercept's interpretation change?

::: {.callout-note collapse="true" title="Solution"}
Only the intercept (and its standard error) changes: it now estimates the fitted reaction time at day 2 rather than day 0. The slope is unchanged because centering is a linear shift of the predictor. This is the practical point of [transformations](../contrasts/transformation.qmd): coding changes interpretation, not fit.

```{julia}
#| eval: false
df = DataFrame(dataset(:sleepstudy))
df.days_c = df.days .- 2
fit(MixedModel, @formula(reaction ~ 1 + days_c + (1 + days_c | subj)), df)
```
:::

2. **Does the random slope earn its keep?** Fit a model with only a random intercept, `(1 | subj)`, and compare it to the random-slope model. Which would you keep, and on what evidence?

::: {.callout-note collapse="true" title="Solution"}
The random-slope model fits substantially better here: subjects differ not just in baseline reaction time but in how strongly sleep deprivation affects them. A likelihood-ratio test makes the comparison explicit.

```{julia}
#| eval: false
sleepstudy = dataset(:sleepstudy)
m_int   = fit(MixedModel, @formula(reaction ~ 1 + days + (1 | subj)), sleepstudy)
m_slope = fit(MixedModel, @formula(reaction ~ 1 + days + (1 + days | subj)), sleepstudy)
MixedModels.likelihoodratiotest(m_int, m_slope)
```
:::
