...
- apply principled model reduction to a larger replication dataset;
- estimate the spatial–attraction correlation parameter with more data and more conditions;
- compare maximal and parsimonious random-effects structures.
:::

::: {.callout-note title="Before you start"}
**Prerequisites:** [Visual Attention Effects: Kliegl et al. (2011)](kwdyz11.qmd).

**Datasets used:** `kkl15` (see the [dataset catalog](../../getting-started/datasets.qmd)).
:::
...
Stroup, W. W. (2012, p. 185). _Generalized linear mixed models: Modern concepts, methods and applications._ CRC Press, Boca Raton.
...
```{julia}
#| echo: false
using SMLP2026: fit_or_restore, bootstrap_or_restore
...
m_max = let
  form = @formula rt ~ 1 + CTR * cardinal * size +
                           (1 + CTR * cardinal | Subj)
  fit_or_restore("kkl15_m_max.json", MixedModel, form, dat; contrasts)
end
```
...
m_zcp1 = let
  form = @formula rt ~ 1 + CTR * cardinal * size +
                  zerocorr(1 + CTR * cardinal | Subj)
  fit_or_restore("kkl15_m_zcp1.json", MixedModel, form, dat; contrasts)
end
```
...
m_zcp1_rdc = let
  form = @formula rt ~ 1 + CTR * cardinal * size +
                   zerocorr(1 + CTR + cardinal | Subj)
  fit_or_restore("kkl15_m_zcp1_rdc.json", MixedModel, form, dat; contrasts)
end
```
...
m_prm1 = let
  form = @formula rt ~ 1 + CTR * cardinal * size +
                           (1 + CTR + cardinal | Subj)
  fit_or_restore("kkl15_m_prm1.json", MixedModel, form, dat; contrasts)
end
```
...
m_cpx = let
  form = @formula rt ~ 1 + CTR * cardinal * size +
                      (1 + CTR + cardinal | Subj)
  fit_or_restore("kkl15_m_cpx.json", MixedModel, form, dat; contrasts)
end
```
...
m_zcp2 = let
  form = @formula rt ~ 1 + CTR * cardinal * size  +
              zerocorr(1 + CTR + cardinal | Subj)
  fit_or_restore("kkl15_m_zcp2.json", MixedModel, form, dat; contrasts)
end
```
...
m_prm2 = let
  form = @formula(rt ~ 1 + CTR * cardinal * size  +
                      (1 + CTR | Subj) + (0 + cardinal | Subj))
  fit_or_restore("kkl15_m_prm2.json", MixedModel, form, dat; contrasts)
end
```
...
caterpillar!(Figure(; size=(800, 1200)), cm1; orderby=2)
```
...
shrinkageplot!(Figure(; size=(1000, 1200)), m_prm1)
```
...
```{julia}
#| include: false
samp = bootstrap_or_restore("kkl15_m_prm1.arrow", MersenneTwister(1234321), 2500, m_prm1;
                            optsum_overrides=(; ftol_rel=1e-8))
```
...
1. **Reduce further.** Starting from the parsimonious model on this page, try removing one more random-effects term. Does the fit get worse in a way a likelihood-ratio test detects, or have you found a defensible simpler model?

::: {.callout-note collapse="true" title="Solution"}
Compare the candidate reduced model to the current one with `likelihoodratiotest`. A non-significant test means the dropped term was not earning its complexity and the simpler model is defensible; a significant test means you have removed something the data support. The goal, per the Stroup quotation in the introduction, is the middle ground between maximal and minimal.
:::

2. **Did the replication deliver?** Compare the spatial–attraction correlation parameter here with the singular estimate in [`kwdyz11`](kwdyz11.qmd). What did the larger sample and wider design buy you?

::: {.callout-note collapse="true" title="Solution"}
With more subjects and more conditions, the critical correlation parameter is estimated away from the ±1 boundary, so the model is non-singular and the parameter becomes interpretable. This is the payoff of the replication: a quantity that was unidentifiable in the original study is now reliably estimated.
:::
