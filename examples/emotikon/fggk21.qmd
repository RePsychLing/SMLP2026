...
**Prerequisites:** [Mixed Models Tutorial: Contrast Coding](../../contrasts/contrasts_fggk21.qmd); [Creating multi-panel plots](../../visualization/AoGPlots.qmd).
...
::: {.callout-tip}
The preprocessing steps above are also packaged as `SMLP2026.fggk21_preprocessed()`, so that other pages (e.g. the [contrast-coding tutorial](../../contrasts/contrasts_fggk21.qmd)) can reuse them without repeating the code.
:::

## Time final LMM of paper
...
## final LMM 
m_test = let
  f_test = @formula zScore ~ 1 + Test*a1*Sex + (1 | School) 
  @time fit(MixedModel, f_test, df; contrasts, progress)
end
```

**Timing the full model with current MixedModels.jl**

```{julia}
#| eval: false
## final LMM 
m_osf = let
  f = @formula zScore ~ 1 + Test*a1*Sex +
     (1+Test+a1+Sex | School) + (1+Test | Child) + 
     zerocorr(1 + Test | Cohort)
  fit(MixedModel, f, df; contrasts, progress)
end
```

Minimizing 4724    Time: 0:23:33 ( 0.30  s/it)

**Timing the full model with new branch pa/gradient-fable**

```{julia}
#Pkg.add MixedModels#pa/gradient-fable
m_osf2 = let
  f = @formula zScore ~ 1 + Test*a1*Sex +
     (1+Test+a1+Sex | School) + (1+Test | Child) + 
     zerocorr(1 + Test | Cohort)
  fit(MixedModel, f, df; contrasts, progress, optimizer=:LD_LBFGS)
end
```

Minimizing 81    Time: 0:01:12 ( 0.90  s/it)

## Extract a stratified subsample

For the purpose of the tutorial, we extract a random sample of 1000 boys and 1000 girls. `Child`, `School`, and `Cohort` are grouping variables.
...
**Stratified sampling:** We build a table with one row per child, sample 1000 boys and 1000 girls from it without replacement, and then extract all of the sampled children's test scores from `df`. Sampling whole children -- rather than individual scores -- preserves the within-child structure that by-`Child` random slopes rely on. This procedure is packaged as `SMLP2026.fggk21_teaching_sample()`; it uses a `StableRNG` with a fixed seed so that the stratification is reproducible across Julia versions (pass a different `seed` for a different pattern of results). The [contrast-coding tutorial](../../contrasts/contrasts_fggk21.qmd) uses the same subsample, so estimates are directly comparable between the two pages.
...
See the [contrast-coding tutorial](../../contrasts/contrasts_fggk21.qmd) for why sequential-difference contrasts are not orthogonal and what that implies for the interpretation of correlation parameters between neighboring contrasts, as well as for alternative contrast schemes for these data; [contrast coding of visual-attention effects](../../contrasts/contrasts_kwdyz11.qmd) works through the general options on a four-level factor.
...
m_ovi = let
  f = @formula zScore ~ 1 + Test * Sex * a1 + (1 | School)
  fit_or_restore("fggk21_m_ovi.json", MixedModel, f, dat; contrasts, progress)
end
```
...
m_zcp = let
  ...
  fit_or_restore("fggk21_m_zcp.json", MixedModel, f, dat; contrasts, progress)
end
```
...
m_cpx = let
  ...
  fit_or_restore("fggk21_m_cpx.json", MixedModel, f, dat; contrasts, progress)
end
```
...
m_cpxCohort = let
  f = @formula zScore ~ 1 + Test * a1 * Sex + (1 + Test | Cohort)
  fit_or_restore("fggk21_m_cpxCohort.json", MixedModel, f, dat; contrasts, progress)
end
```
...
The model is overparameterized, with correlation parameters estimated close to ±1.
...
m_zcpCohort = let
  ...
  fit_or_restore("fggk21_m_zcpCohort.json", MixedModel, f, dat; contrasts, progress)
end
```
...
This `zcpLMM` is also singular: with the present subsample, all five VCs are estimated as zero.
...
m_oviCohort = let
  f = @formula zScore ~ 1 + Test * a1 * Sex + (1 | Cohort)
  fit_or_restore("fggk21_m_oviCohort.json", MixedModel, f, dat; contrasts, progress)
end
```
...
:::{.callout-warning}
The following LMMs `m1`, `m2`, etc. take a bit longer to fit (a few minutes each on a recent laptop).
As elsewhere in these materials, the rendered page restores the fits from a cache via `fit_or_restore`, so re-rendering is fast; delete the corresponding files under `fits/` to force a fresh fit.
:::
...
m1 = let
  ...
  fit_or_restore("fggk21_m1.json", MixedModel, f, dat; contrasts, progress)
end
```
...
m2 = let
  ...
  fit_or_restore("fggk21_m2.json", MixedModel, f, dat; contrasts, progress)
end
```
...
See @Fuehner2021 for an application.
...
Returning to the theoretical focus of the article, the significant main effects of `age` and `Sex` are replicated, as are the interaction between `age` and the c4 contrast and the interactions between `Sex` and two test contrasts (c1, c4).
The remaining contrast interactions reported for the full data do not reach significance in this subsample.
...
m2_nested = let
  ...
  fit_or_restore("fggk21_m2_nested.json", MixedModel, f, dat; contrasts, progress)
end
```
...
- [MixedModels.jl cheatsheet](../../reference/mixedmodels_cheatsheet.qmd) — a tour of the extractor functions for inspecting a fitted model, formerly a section of this page.
...
2. **Add a comparison.** Add a contrast for the `Test` factor that encodes a comparison you find substantively interesting, refit, and check whether it changes the conclusions about the age-by-sex interaction.

::: {.callout-note collapse="true" title="Solution"}
Because re-coding a factor does not change the model fit, the omnibus conclusions (and the age-by-sex interaction) are unaffected; what changes is which specific comparison each `Test` coefficient reports. Use this to confirm that your substantive comparison is or is not significant without disturbing the rest of the model. See [Mixed Models Tutorial: Contrast Coding](../../contrasts/contrasts_fggk21.qmd).
:::
