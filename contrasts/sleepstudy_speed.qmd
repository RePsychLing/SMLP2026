...
- re-express reaction time as speed and discuss when a response transformation aids interpretation;
- compare within-subject simple regressions with a mixed-model fit;
- build LMMs with and without correlation parameters and compare between-group effects.
:::

::: {.callout-note title="Before you start"}
**Prerequisites:** [Analysis of the sleepstudy data](../lmm-intro/sleepstudy.qmd); [Transformations of the predictors and the response](transformation.qmd).
...
The `sleepstudy` data are one of the datasets available with course materials or with the `MixedModelsDatasets` package. We carry out some preprocessing to have the dataframe in the desired shape:

  - Capitalize random factor `Subj`
  - Compute `speed` as an alternative dependent variable from `reaction`, warranted by a Box-Cox check of residuals.
...
gdf = @chain dataset(:sleepstudy) begin
  DataFrame(_)
  rename!(_, :subj => :Subj, :days => :day)
  @transform!(_, :speed = 1000 / :reaction)
  groupby(_, :Subj)
end
```
...
```{julia}
pooledcoef = simplelinreg(df.day, df.speed)
```
...
let
  ...
  f = clevelandaxes!(Figure(; size=(1000, 750)), labs, (2, 9))
  for (axs, sdf) in zip(f.content, gdf[ord]) # iterate over the panels and groups
    ...
    ablines!(axs, first(coef), last(coef))  # add the regression line
  end
  f
end
```
...
```{julia}
#| eval: false
m1 = lmm(@formula(speed ~ 1 + day + (1 + day | Subj)), df)
println(m1) # force plain text output
```
...
m1 = let
  ...
  fit_or_restore("sleepstudy_speed_m1.json", MixedModel, form, df)
end
println(m1)
```
...
```{julia}
#| eval: false
m2 = lmm(@formula speed ~ 1 + day + zerocorr(1 + day | Subj), df)
println(m2)
```
...
m2 = let
  ...
  fit_or_restore("sleepstudy_speed_m2.json", MixedModel, form, df)
end
println(m2)
```
...
caterpillar!(Figure(; size=(500, 500)), m2)
```
...
shrinkageplot!(Figure(; size=(400, 400)), m2)
```

# See Also

- [Analysis of the sleepstudy data](../lmm-intro/sleepstudy.qmd) — the reaction-time analysis of these data that this page re-examines on the speed scale.
- [Shrinkage of random effects](../visualization/shrinkageplot.qmd) — what the caterpillar and shrinkage plots above are showing and how to read them.
...
1. **Speed versus time.** Refit the central model using speed (`1000 / reaction`, responses per second) instead of reaction time. How does the direction and interpretation of the sleep-deprivation effect change?

::: {.callout-note collapse="true" title="Solution"}
On the speed scale the effect of deprivation has the opposite sign — performance (responses per second) *decreases* with more days of restriction, where reaction time *increases*. The substantive story is the same; only the units and sign flip. Speed is sometimes preferred because its residuals are better behaved than those of raw reaction time.
:::

2. **Correlation parameter.** Compare the full model with a zero-correlation-parameter (`zerocorr`) version. Does dropping the correlation parameter materially change the fixed-effect conclusions?

::: {.callout-note collapse="true" title="Solution"}
Usually the fixed-effect estimates and their standard errors are nearly unchanged when you drop a weakly-identified correlation parameter, while the model becomes easier to fit and less prone to singularity. Compare them with `likelihoodratiotest` and inspect whether the correlation was well determined in the first place.
:::
