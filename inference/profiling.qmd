...
- profile the objective function of a fitted model to assess parameter variability;
- read profile zeta plots;
- compare profile-based confidence intervals with the parametric bootstrap.
:::

::: {.callout-note title="Before you start"}
**Prerequisites:** [Analysis of the sleepstudy data](../lmm-intro/sleepstudy.qmd); [Parametric bootstrap for mixed-effects models](bootstrap.qmd) is helpful for the comparison.
...
pr01 = let f = @formula(1000 / rt_raw ~
  ...
  profile(lmm(f, kb07; contrasts, progress))
end
...
pr01 = let f = @formula(1000 / rt_raw ~
    1 + load + spkr + prec + (1 + prec | item) + (1 | subj))
  profile(fit_or_restore("profiling_pr01.json", MixedModel, f, kb07; 
                          contrasts, progress))
end
println(pr01.m) # model is a property of the profile object
```
...
zetaplot!(Figure(; size=(1200, 350)), pr01; ptyp='β')
```
...
zetaplot!(Figure(; size=(1200, 330)), pr01; ptyp='β', absv=true)
```
...
zetaplot!(Figure(; size=(1200, 330)), pr01; ptyp='σ', absv=true)
```
...
zetaplot!(Figure(; size=(1200, 330)), pr01; ptyp='θ', absv=true)
```
...
profiledensity!(Figure(; size=(1200, 300)), pr01; share_y_scale=false)
```
...
zetaplot!(Figure(; size=(1100, 330)), pr01; ptyp='σ')
```
...
- [Parametric bootstrap for mixed-effects models](bootstrap.qmd) — the simulation-based alternative to profiling.
- The [introductory chapter](https://www.embraceuncertaintybook.com/intro.html) of *Embrace Uncertainty* — develops the profiled log-likelihood from first principles on the simplest possible example.

## Exercises

1. **Profile versus bootstrap.** Compute profile-based confidence intervals for the `kb07` model and compare them to the parametric-bootstrap intervals. Where do they agree, and where might they diverge?

::: {.callout-note collapse="true" title="Solution"}
Profile and bootstrap intervals usually agree closely for well-identified fixed effects. They diverge most for variance components and correlation parameters near a boundary, where the profile's asymmetry and the bootstrap's mass at the boundary both signal that a symmetric standard-error interval would be misleading.
:::

2. **Read a zeta plot.** In a profile zeta plot, what does a curve that is markedly nonlinear (rather than a straight line) tell you about that parameter's confidence interval?

::: {.callout-note collapse="true" title="Solution"}
A straight zeta profile means the likelihood is well approximated by a quadratic, so a symmetric Wald (standard-error) interval is adequate. A curved profile means the interval is asymmetric — the parameter is better constrained on one side than the other — and the profile- or bootstrap-based interval should be preferred over a symmetric one.
:::
