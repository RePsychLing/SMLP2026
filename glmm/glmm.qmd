...
**Prerequisites:** [Analysis of the sleepstudy data](../lmm-intro/sleepstudy.qmd); the [Building Models](../model-specification/partial_within.qmd) section is helpful.

**Datasets used:** `sleepstudy` and `contra` (see the [dataset catalog](../getting-started/datasets.qmd)).
:::
...
#| include: false
m1 = let f = @formula reaction ~ 1 + days + (1 + days | subj)
  fit_or_restore("glmm_m1.json", MixedModel, f, sleepstudy; progress)
end
println(m1)
```
...
<!-- add note about how we determined this! --> 
...
- The [binary-response GLMM chapter](https://www.embraceuncertaintybook.com/glmmbernoulli.html) of *Embrace Uncertainty* — a full treatment of the same contraception data, including link functions and interpretation on the odds scale.
...
2. **Link and distribution.** What two choices distinguish a GLMM from an LMM, and which part of the `fit(MixedModel, ...)` call encodes them?
