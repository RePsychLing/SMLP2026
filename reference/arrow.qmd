...
The [MixedModelsDatasets](https://juliamixedmodels.github.io/MixedModelsDatasets.jl/stable/) package uses Arrow to store all of its datasets.
...
fn = joinpath(DATADIR, "emotikon.rds")
if !isfile(fn)
  @info "Downloading Emotikon data"
...
```{julia}
arrowfn = joinpath(DATADIR, "fggk21.arrow")
...
```{julia}
filesize(
  Arrow.write(joinpath(DATADIR, "fggk21_Child.arrow"), Child; compress=:lz4)
)
```
...
filesize(
  Arrow.write(
    joinpath(DATADIR, "fggk21_Score.arrow"),
...
```{julia}
Score = DataFrame(Arrow.Table(joinpath(DATADIR, "fggk21_Score.arrow")))
```
...
```{julia}
Child = DataFrame(Arrow.Table(joinpath(DATADIR, "fggk21_Child.arrow")))
```
...
read_table("fggk21.arrow")
```

```{r}
#| eval: false
library(arrow)
read_ipc_file("fggk21.arrow")
```

# See Also

- The [data tables appendix](https://www.embraceuncertaintybook.com/datatables.html) of *Embrace Uncertainty* — Arrow files, memory-mapped data, and table representations in Julia.
