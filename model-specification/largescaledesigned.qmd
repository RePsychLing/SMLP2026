---
title: "Large-Scale Designed Experiments: The English Lexicon Project"
engine: julia
julia:
...
- extract analysis-ready tables from a large raw dataset;
- explore and model trial-level data from a large designed experiment (the English Lexicon Project);
- fit models with scalar random effects at scale and relate the random effects to covariates.
:::

::: {.callout-note title="Before you start"}
**Prerequisites:** [Analysis of the sleepstudy data](../lmm-intro/sleepstudy.qmd); [Model Specification and Selection](mrk17.qmd).

**Datasets used:** the English Lexicon Project lexical-decision data (`ELP_ldt_trial`, `ELP_ldt_item`, `ELP_ldt_subj`; see the [dataset catalog](../getting-started/datasets.qmd)).
:::
...
If you examine the code for @fig-elpldtspeeddens, you will see that the conversion from `rt` to speed is done inline rather than creating and storing a new variable in the DataFrame.
...
elm01 = let
  ...
  fit_or_restore("largescaledesigned_elm01.json", MixedModel, form, pruned; contrasts, progress=false)
end
```
...
elm02 = let
  ...
  dat = @subset(pruned, :spropacc > 0.8)
  fit_or_restore("largescaledesigned_elm02.json", MixedModel, form, dat; contrasts, progress=false)
end
```
...
```{julia}
if !isfile(joinpath(FITSDIR, "elm01.json"))
  saveoptsum(joinpath(FITSDIR, "elm01.json"), elm01);
end
```
...
1. **Why scalar random effects first?** At this scale, why is it sensible to start with scalar (intercept-only) random effects for subject and item before adding random slopes?

::: {.callout-note collapse="true" title="Solution"}
Scalar random effects are cheap to estimate and almost always well supported, so they give a stable, fast baseline that confirms the data pipeline and the fixed-effects structure before you spend computation on the much larger vector-valued random-effects models. They also provide a reference point for judging whether added complexity actually improves the fit.
:::

2. **Relate random effects to covariates.** Extract the conditional modes of the by-item intercepts and check whether they are related to an item-level covariate. What would a strong relationship suggest?

::: {.callout-note collapse="true" title="Solution"}
A clear relationship between the by-item random intercepts and an item-level covariate suggests that covariate should be added as a *fixed* effect: the random intercepts are absorbing systematic, explainable variation that a predictor could capture directly. Use `raneftables` (or `ranef`) to pull the conditional modes and join them to the item covariates.
:::
