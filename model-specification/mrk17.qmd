---
title: "Model Specification and Selection: Masson, Rabe & Kliegl (2017)"
authors:
- "Reinhold Kliegl"
- "Phillip Alday"
engine: julia
...
- translate an experimental design into a mixed-model specification (response, covariates, factors, grouping);
- distinguish between-unit and within-unit factors and map them to fixed and random effects;
- reason about the variance components and correlation parameters implied by a factorial design.
:::

::: {.callout-note title="Before you start"}
**Prerequisites:** [Analysis of the sleepstudy data](../lmm-intro/sleepstudy.qmd); [Transformations of the predictors and the response](../contrasts/transformation.qmd) is helpful.

**Datasets used:** `mrk17` (the `mrk17_exp1` dataset; see the [dataset catalog](../getting-started/datasets.qmd)).
:::
...
The data (variables and observations) used by Masson et al. (2017) are available in the dataset `:mrk17_exp1`:
...
`lQ` and `lT` refer to experimental conditions in the last trial.
...
m_cpx = let
    form = @formula((1000/rt) ~ 1+F*P*Q*lQ*lT +
                               (1+F+P+Q+lQ+lT | subj) +
                               (1  +P+Q+lQ+lT | item))
    fit(MixedModel, form, dat; contrastsf, progress)
end
...
m_cpx = let
    form = @formula((1000/rt) ~ 1+F*P*Q*lQ*lT +
                               (1+F+P+Q+lQ+lT | subj) +
                               (1  +P+Q+lQ+lT | item))
    fit_or_restore("mrk17_m_cpx.json", MixedModel, form, dat; contrasts, progress)
end
```
...
VP is approximately zero for fourth diagonal entry; not supported by data.
...
m_zcp = let
    form = @formula((1000/rt) ~ 1+F*P*Q*lQ*lT +
                       zerocorr(1+F+P+Q+lQ+lT | subj) +
                       zerocorr(1  +P+Q+lQ+lT | item))
    fit_or_restore("mrk17_m_zcp.json", MixedModel, form, dat; contrasts, progress)
end
```
...
m_zcp2 = let
    form = @formula((1000/rt) ~ 1+F*P*Q*lQ*lT +
                       zerocorr(1  +P+Q+lQ+lT | subj) +
                       zerocorr(1  +P+Q   +lT | item))
    fit_or_restore("mrk17_m_zcp2.json", MixedModel, form, dat; contrasts, progress)
end
```
...
m_zcp3 = let
    form = @formula((1000/rt) ~ 1+F*P*Q*lQ*lT +
                       zerocorr(1    +Q   +lT | subj) + (1 | item))
    fit_or_restore("mrk17_m_zcp3.json", MixedModel, form, dat; contrasts, progress)
end
```
...
m_zcp4 = let
    form = @formula((1000/rt) ~ 1+F*P*Q*lQ*lT +
                       zerocorr(1         +lT | subj) + (1 | item))
    fit_or_restore("mrk17_m_zcp4.json", MixedModel, form, dat; contrasts, progress)
end
```
...
m_prm = let
    form = @formula((1000/rt) ~ 1+F*P*Q*lQ*lT +
                               (1+    Q+lT | subj) + (1 | item))
    fit_or_restore("mrk17_m_prm.json", MixedModel, form, dat; contrasts, progress)
end
```
...
m_mrk17_crossed =let
   form = @formula (1000/rt) ~ 1 + F*P*Q*lQ*lT +
        (1+Q | subj) + zerocorr(0+lT | subj) + zerocorr(1 + P | item) ;
    fit_or_restore("mrk17_m_mrk17_crossed.json", MixedModel, form, dat; contrasts, progress)
end
```
...
m_mrk17_nested =let
   form = @formula (1000/rt) ~ 1 + Q/(F/P) +
        (1+Q | subj) + zerocorr(0+lT | subj) + zerocorr(1 + P | item) ;
    fit_or_restore("mrk17_m_mrk17_nested.json", MixedModel, form, dat; contrasts, progress)
end
```

## See Also

- The [multiple random-effects terms chapter](https://www.embraceuncertaintybook.com/multiple.html) of *Embrace Uncertainty* — a deeper treatment of crossed, nested, and partially crossed random-effects structures.
- The [large-scale designed experiment chapter](https://www.embraceuncertaintybook.com/largescaledesigned.html) of *Embrace Uncertainty* — model specification and selection at scale, on the English Lexicon Project.

## Exercises

1. **Map the design.** List the experimental factors in `mrk17` and label each as between- or within-subject and between- or within-item. Which factors are therefore eligible for by-subject and by-item random slopes?

::: {.callout-note collapse="true" title="Solution"}
The relatedness, frequency, and quality manipulations are crossed with both subjects and items in this repeated-measures design, so they vary within subjects and within items and are eligible for both by-subject and by-item random slopes. Factors that are properties of the item (e.g. target frequency) vary between items but within subjects, so they support a by-subject slope but not a by-item slope. Writing this table out is the first step in choosing a defensible random-effects structure.
:::

2. **Write the maximal model.** Based on your classification, write the formula for the maximal random-effects structure, then a more parsimonious alternative using `zerocorr`.

::: {.callout-note collapse="true" title="Solution"}
The maximal model gives every within-grouping factor (and their interactions) a random slope for both `subj` and `item`; the parsimonious version wraps the random-effects term in `zerocorr(...)` to drop the correlation parameters that are typically the hardest to estimate. See [Convergence, singularity and all that](../lmm-intro/singularity.qmd) for why the maximal model is often singular in practice.
:::
