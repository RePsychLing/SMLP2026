...
**Prerequisites:** [Analysis of the sleepstudy data](../lmm-intro/sleepstudy.qmd) for the modelling vocabulary; see also [Useful packages](../reference/useful_packages.qmd) for the plotting stack.
...
1. **Add an aesthetic.** Extend the plot by mapping a grouping variable (for example `Sex`) to colour. How does the `mapping(...)` call change?

::: {.callout-note collapse="true" title="Solution"}
Add the variable as a `color` argument to `mapping`. AlgebraOfGraphics then draws one coloured series per level and adds a legend automatically.

```{julia}
#| eval: false
# inside the existing plot, change the mapping to something like:
mapping(:age, :score; color=:Sex, layout=:Test)
```
:::

2. **Save the figure.** Write the final figure to a PNG file at a chosen resolution.
