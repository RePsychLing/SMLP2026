...
1. **Add an aesthetic.** Extend the plot by mapping a grouping variable (for example `Sex`) to color. How does the `mapping(...)` call change?

::: {.callout-note collapse="true" title="Solution"}
Add the variable as a `color` argument to `mapping`. AlgebraOfGraphics then draws one colored series per level and adds a legend automatically.
